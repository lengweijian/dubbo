...
/**
 * callback com.atlwj.service helper
 */
class CallbackServiceCodec {
    ...
    /**
     * export or unexport callback com.atlwj.service on client side
     *
     * @param channel
     * @param url
     * @param clazz
        ...
        if (export) {
            ...
            if (!channel.hasAttribute(cacheKey)) {
                if (!isInstancesOverLimit(channel, url, clazz.getName(), instid, false)) {
                    ...
                    // this is used for tracing if instid has published com.atlwj.service or not.
                    channel.setAttribute(cacheKey, exporter);
                    logger.info("Export a callback com.atlwj.service :" + exportUrl + ", on " + channel + ", url is: " + url);
    ...
    /**
     * refer or destroy callback com.atlwj.service on server side
     *
     * @param url
     */
        ...
        if (isRefer) {
            if (proxy == null) {
                ...
                if (!isInstancesOverLimit(channel, referurl, clazz.getName(), instid, true)) {
                    ...
                    logger.info("method " + inv.getMethodName() + " include a callback com.atlwj.service :" + invoker.getUrl() + ", a proxy :" + invoker + " has been created.");
                }
            }
        } else {
            if (proxy != null) {
    ...
    private static boolean isInstancesOverLimit(Channel channel, URL url, String interfaceClass, int instid, boolean isServer) {
        ...
        if (count != null && count >= limit) {
            ...
            throw new IllegalStateException("interface " + interfaceClass + " `s callback instances num exceed providers limit :" + limit
                    + " ,current num: " + (count + 1) + ". The new callback com.atlwj.service will not work !!! you can cancle the callback com.atlwj.service which exported before. channel :" + channel);
        } else {
            return false;
        }
    }
