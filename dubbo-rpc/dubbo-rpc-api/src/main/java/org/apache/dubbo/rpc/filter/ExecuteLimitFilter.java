...
/**
 *
 * The maximum parallel execution request count per method per com.atlwj.aop.service for the provider.If the max configured
        ...
        if (!RpcStatus.beginCount(url, methodName, max)) {
            throw new RpcException(RpcException.LIMIT_EXCEEDED_EXCEPTION,
                    "Failed to invoke method " + invocation.getMethodName() + " in provider " +
                            url + ", cause: The com.atlwj.aop.service using threads greater than <dubbo:com.atlwj.aop.service executes=\"" + max +
                            "\" /> limited.");
        }
